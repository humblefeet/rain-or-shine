...
function  randomizeCategories(){
    sunCategory = sun[Math.floor(Math.random()*sun.length)];
    rainCategroy = rain[Math.floor(Math.random()*rain.length)]
}
...
router.get("/weather", function(req, res) {
    ...
    request(weatherUrl, function(err, response, body) {
        ...
        console.log(`Temperatture is ${temperature}`)
        console.log(`Precipitation is ${precipitation}`)
        var categories;

        if (precipitation > 50){ 
            categories= rain[Math.floor(Math.random()*rain.length)]        
        }else if (precipitation < 50 && precipitation >25 && temperature > 65){
            categories = sun[Math.floor(Math.random()*sun.length)]+'&categoryId='+rain[Math.floor(Math.random()*rain.length)];
        }else if (precipitation < 25  && temperature > 50){
            categories= sun[Math.floor(Math.random()*sun.length)]
        }
        ...
        request(foursquareUrl, function(err, response, venues) {
            var superData=  Object.assign({},JSON.parse(venues),(weather))
            res.json(superData)
        })
    })
})
